      ...
      <Dialog.Portal>
        ...
        >
          <div className="flex items-center justify-between border-b border-dq-border px-5 py-4">
            <Dialog.Title className="type-title text-dq-black">Your Gift</Dialog.Title>
            <Dialog.Close
              className="rounded-full p-2 text-dq-black transition-colors hover:bg-dq-cream"
              aria-label="Close your gift"
            >
              <X className="h-5 w-5" />
            </Dialog.Close>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5">
            {cart.items.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
                <p className="type-body text-dq-muted">Your gift is empty. Add a sponsorship to begin.</p>
          ...
          {cart.items.length > 0 ? (
            <div className="border-t border-dq-border px-5 py-4">
              ...
              <p className="mb-4 text-xs text-dq-muted">
                {itemCount} {itemCount === 1 ? 'item' : 'items'} · Payment completes your donation
              </p>
